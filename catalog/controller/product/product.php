        ...
        if ($product_info) {
            ...
            foreach ($results as $result) {
                $data['images'][] = array(
                    ...
                    'thumb' => $result['image'] // $this->model_tool_image->resize($result['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'))
                );
            }
            ...
            foreach ($results as $result) {
                ...
                if ($result['image']) {
                    $image = $server . 'image/' . $result['image']; //$this->model_tool_image->resize($result['image'], $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
                } else {
                    $image = 'placeholder.png'; //$this->model_tool_image->resize('placeholder.png', $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
                }
                ...
                $popupOptions = $this->model_catalog_product->getProductOptions($result['product_id']);

                foreach ($popupOptions as $option) {
                    $product_option_value_data2 = array();
                    if (!strcmp($option['name'], "سایز") || !strcmp($option['name'], "size")) {
                        foreach ($option['product_option_value'] as $option_value) {
                            if (!$option_value['subtract'] || ($option_value['quantity'] >= 0)) {
                                $product_option_value_data2[] = array(
                        ...
                        $data['popupOptions'][] = array(
                            'product_option_id' => $option['product_option_id'],
                            'product_option_value' => $product_option_value_data2,
                            'option_id' => $option['option_id'],
                            'name' => $option['name'],
                            'type' => $option['type'],
                            'value' => $option['value'],
