...
class ModelCatalogProduct extends Model
{
	...
	{
		$sql = "select * from ( SELECT p.product_id,p.price, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
      ...
      p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		...
		if (!empty($data['filter_category_id'])) {
			...
			if (!empty($data['filter_filter'])) {
				$filter_groups = $this->db->query("select filter_group_id, GROUP_CONCAT(filter_id SEPARATOR ',') as filter_groups from " . DB_PREFIX . "filter
         where filter_id in (" . $data['filter_filter'] . ") group by filter_group_id")->rows;
				...
				foreach ($filter_groups as $filter_group) {
					...
					$sql .= " AND pf.product_id IN (SELECT dpf.product_id FROM dwc_product_filter dpf WHERE dpf.filter_id IN (" . $filter_group['filter_groups'] . ")) ";
				}
			}
		}
...
//      }
		...
		if (!empty($data['minPrice'])) {
			...
			$sql .= " and IF(bigh.special  is NOT null , bigh.special, bigh.price) >= '$minPrice' ";
		}
		if (!empty($data['maxPrice'])) {
			$maxPrice = $data['maxPrice'];
			$sql .= " and IF(bigh.special  is NOT null , bigh.special, bigh.price) <= '$maxPrice' ";
		}
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			$product_option_value_query =
				$this->db->query(
                    ...
                    WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_id = '" .
